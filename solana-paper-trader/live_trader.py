...
class JupiterClient:
    ...
    def execute(
        ...
        if not isinstance(payload, dict):
            raise LiveBotError(
                "Jupiter returned an invalid execute response"
            )
...
class PrivySigner:
    def __init__(
        ...
        if not app_id or not app_secret or not wallet_id:
            raise FatalLiveBotError(
                "Missing PRIVY_APP_ID, PRIVY_APP_SECRET or PRIVY_WALLET_ID"
            )
    ...
    def wallet(self) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise FatalLiveBotError(
                "Privy returned an invalid wallet response"
            )

        if payload.get("chain_type") != "solana":
            raise FatalLiveBotError(
                "PRIVY_WALLET_ID is not a Solana wallet"
            )
    ...
    def sign_transaction(
        ...
        if not signed:
            raise LiveBotError(
                "Privy returned an empty signed transaction"
            )
...
class LiveTrader:
    ...
    def sol_balance_lamports(
        ...
        try:
            ...
            raise LiveBotError(
                f"Invalid Solana balance response: {result}"
            ) from error
    ...
    def _route_safety(
        ...
        if (
            ...
            reasons.append(
                f"buy_price_impact_"
                f"{buy_impact:.2f}_pct"
            )

        sell_quote = self.jupiter.order(
            mint,
            SOL_MINT,
            buy_out,
        )

        sell_out = self._amount(
            sell_quote,
            "outAmount",
        )
        ...
        if (
            ...
            reasons.append(
                f"sell_price_impact_"
                f"{sell_impact:.2f}_pct"
            )
        ...
        if sell_out > 0:
            ...
            if (
                roundtrip_pct
                < self.config.min_roundtrip_return_pct
            ):
                reasons.append(
                    f"roundtrip_quote_"
